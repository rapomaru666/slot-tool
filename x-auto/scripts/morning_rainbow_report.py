...
def find_result_text(html, hall):
    ...
    for pat, label in [
        ...
        if m:
            nums.append(f"{label}{m.group(1)}{'%' if label == '勝率' else ('G' if label == '平均G' else '枚')}")
...
def main():
    ...
    for hall in halls:
        ...
        if found:
            ...
            body = "現時点では公開データから実績数値を確認できず。\n📣当日の状況・差枚など分かる有志の方、情報提供お願いします🙇\nリプで教えていただけると助かります！"
        text = f"🌈Result {target.month}/{target.day}\n🌈 {hall}\n{body}\n#スロット #パチスロ"
        result, posted_text = publish(text, channel)
        posts.append({"hall": hall, "text": posted_text, "chars": len(posted_text), "buffer": result, "source": source})
        if source:
    ...
    print(json.dumps({"ok": True, "target": target.isoformat(), "post_count": len(posts), "posts": [{"hall": p["hall"], "chars": p["chars"]} for p in posts]}, ensure_ascii=False))
