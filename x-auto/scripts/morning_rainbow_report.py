...
def graphql(query: str):
    ...
    req = urllib.request.Request(API_URL, data=json.dumps({"query": query}).encode(), headers={"Content-Type":"application/json","Authorization":f"Bearer {token}"}, method="POST")
...
def fetch(url):
    req=urllib.request.Request(url, headers={"User-Agent":"Mozilla/5.0 (compatible; RAPOMARU-X-Auto/1.0)","Accept-Language":"ja,en;q=0.8"})
    with urllib.request.urlopen(req, timeout=30) as res: return res.read().decode("utf-8", errors="replace")


def rainbow_halls(thread):
    halls=[]; active=False
    for line in thread.get("root","").splitlines():
        if line.strip()=="🌈": active=True; continue
        if line.strip()=="🏆": active=False; continue
        if active and "｜" in line: halls.append(line.split("｜",1)[0].strip())
    return halls
...
def find_result_text(html, hall):
    ...
    chunk=text[p:p+900]
    nums=[]
    for pat,label in [(r"平均[^+\-\d]{0,12}([+\-]?\d[\d,]*)\s*枚","平均差枚"),(r"勝率[^\d]{0,8}(\d+(?:\.\d+)?)%","勝率"),(r"平均[^\d]{0,12}(\d[\d,]*)\s*G","平均G")]:
        m=re.search(pat,chunk)
        if m: nums.append(f"{label}{m.group(1)}{'%' if label=='勝率' else ('G' if label=='平均G' else '枚')}")
...
def main():
    ...
    if today < START_DATE:
        print("Morning reports have not started yet"); return
    target=today-timedelta(days=1)
    thread_path=Path(f"x-auto/thread-{target.isoformat()}.json")
    if not thread_path.exists(): raise RuntimeError(f"Prediction thread missing: {thread_path}")
    thread=json.loads(thread_path.read_text(encoding="utf-8"))
    halls=rainbow_halls(thread)
    if not halls:
        print("No rainbow halls; no morning report required"); return
    history=json.loads(PUBLISHED_PATH.read_text(encoding="utf-8")) if PUBLISHED_PATH.exists() else {}
    ...
    for url in urls:
        for attempt in range(3):
            try: pages.append((url,fetch(url))); break
            except Exception:
                if attempt==2: pass
                else: time.sleep(5)
