document.addEventListener("DOMContentLoaded",()=>{
  const state={at:0,cz:0,strongCherry:0,strongCherryHit:0};
  const rates={
  ...
  function poissonLog(games,count,rate){
    if(games<=0||!rate)return 0;
    const lambda=games/rate;
    return count===0?-lambda:count*Math.log(lambda)-lambda-logFactorial(count);
  }

  function binomialLog(n,k,p){
    if(n<=0)return 0;
  ...
  function allowedSettings(){
    ...
    document.querySelectorAll("button.signal.selected").forEach(btn=>{
      if(btn.dataset.min){
        ...
        allowed=new Set([...allowed].filter(s=>s>=min));
      }
      if(btn.dataset.only){
        const only=new Set(btn.dataset.only.split(",").map(Number));
        allowed=new Set([...allowed].filter(s=>only.has(s)));
      }
      if(btn.dataset.exclude){
        allowed.delete(Number(btn.dataset.exclude));
      }
    });
    return allowed;
  }
  ...
  function probabilities(games){
    ...
    for(let i=0;i<6;i++){
      ...
      if(state.strongCherry>0){
        log+=binomialLog(state.strongCherry,state.strongCherryHit,rates.strongCherry[i]);
      }
      logs.push(log);
    }
  ...
  function render(p,games){
    ...
    if(summary){
      summary.textContent=signals.length
        ?`参考示唆：${signals.join(" / ")} ※出現率未公表のため数値％には加算していません。`
        :"";
    }
  }
  ...
  $("judge-btn")?.addEventListener("click",()=>{
    ...
    if(state.strongCherryHit>state.strongCherry){
      alert("強チェリー当選回数が強チェリー回数を超えています。");
      return;
    }
